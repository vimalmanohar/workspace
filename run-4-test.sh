...
##
####################################################################
decode=exp/tri3/decode_${dirid}
if [ ! -f ${decode}/.done ]; then
  echo ---------------------------------------------------------------------
  echo "Spawning decoding with CDHMM models  on" `date`
  echo ---------------------------------------------------------------------
  utils/mkgraph.sh \
  ...
  steps/decode.sh --skip-scoring true --beam 10 --lattice-beam 4\
    --nj $my_nj --cmd "$decode_cmd" "${decode_extra_opts[@]}"\
    exp/tri3/graph ${datadir} ${decode} |tee ${decode}/decode.log
  touch ${decode}/.done
fi
...
## SGMM decoding 
####################################################################
decode=exp/sgmm3/decode_${dirid}
if [ ! -f $decode/.done ]; then
  echo ---------------------------------------------------------------------
  echo "Spawning $decode with SGMM on" `date`
  echo ---------------------------------------------------------------------
  utils/mkgraph.sh \
    data/lang exp/sgmm3 exp/sgmm3/graph |tee exp/sgmm3/mkgraph.log

  mkdir -p $decode
  steps/decode_sgmm.sh --skip-scoring true --use-fmllr false --nj $my_nj \
    --cmd "$decode_cmd" \
    exp/sgmm3/graph ${datadir} $decode |tee $decode/decode.log
  touch $decode/.done
fi
...
if ! $fast_path ; then
  local/run_kws_stt_task.sh --cer $cer --max-states $max_states \
    --cmd "$decode_cmd" --skip-kws $skip_kws --skip-stt $skip_stt --wip $wip \
    "${shadow_set_extra_opts[@]}" "${lmwt_plp_extra_opts[@]}" \
    ${datadir} data/lang  exp/sgmm3/decode_${dirid}
fi
...
## PhoneTxCAT decoding 
####################################################################
decode=exp/phoneTxCAT3/decode_${dirid}
if [ ! -f $decode/.done ]; then
  echo ---------------------------------------------------------------------
  echo "Spawning $decode with PhoneTxCAT Model on" `date`
  echo ---------------------------------------------------------------------
  utils/mkgraph.sh \
    data/lang exp/phoneTxCAT3 exp/phoneTxCAT3/graph |tee exp/phoneTxCAT3/mkgraph.log

  mkdir -p $decode
  steps/decode_phoneTxCAT.sh --skip-scoring true --nj $my_nj \
    --cmd "$decode_cmd" "${decode_extra_opts[@]}"\
    exp/phoneTxCAT3/graph ${datadir} $decode |tee $decode/decode.log
  touch $decode/.done
fi
...
if ! $fast_path ; then
  local/run_kws_stt_task.sh --cer $cer --max-states $max_states \
    --cmd "$decode_cmd" --skip-kws $skip_kws --skip-stt $skip_stt --wip $wip \
    "${shadow_set_extra_opts[@]}" "${lmwt_plp_extra_opts[@]}" \
    ${datadir} data/lang  exp/phoneTxCAT3/decode_${dirid}
fi
